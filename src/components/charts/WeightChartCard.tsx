      ...
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Badge variant="secondary" className="tabular-nums text-sm font-semibold">
              {latest.weight.toLocaleString('ro-RO', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} kg
            </Badge>
            {delta != null && (
              <Badge variant={delta <= 0 ? 'success' : 'warning'} className="tabular-nums">
                {delta > 0 ? '+' : ''}{delta.toLocaleString('ro-RO', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} kg
              </Badge>
            )}
            {toTarget != null && (
              <Badge variant="outline" className="tabular-nums">
                {toTarget > 0 ? '+' : ''}{toTarget.toLocaleString('ro-RO', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} kg față de țintă
              </Badge>
            )}
          </div>
        </div>
                ...
                return (
                  <TableRow key={row.date}>
                    ...
                    <TableCell className="text-right font-semibold tabular-nums">
                      {row.weight.toLocaleString('ro-RO', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} kg
                    </TableCell>
                    <TableCell className="text-right tabular-nums text-muted-foreground">
                      {diff == null ? '—' : (
                        <span className={diff <= 0 ? 'text-green-600 dark:text-green-400' : 'text-amber-600 dark:text-amber-400'}>
                          {diff > 0 ? '+' : ''}{diff.toLocaleString('ro-RO', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} kg
                        </span>
                      )}
                    </TableCell>
                  </TableRow>
